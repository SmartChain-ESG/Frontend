      ...
      <div className="flex flex-col gap-[24px] p-[24px] lg:p-[40px] max-w-[900px] mx-auto w-full">
        ...
        <div className="bg-white rounded-[12px] border border-[var(--color-border-default)] p-[24px]">
          <div className="grid grid-cols-2 gap-[20px]">
            ...
            <InfoRow label="요청일" value={formatKoreanDate(approval.requestedAt)} />
            {approval.processedAt && (
              <InfoRow label="처리일" value={formatKoreanDate(approval.processedAt)} />
            )}
          </div>
          ...
          <div className="bg-white rounded-[12px] border border-[var(--color-border-default)] p-[24px]">
            ...
            <ol className="relative border-l-[2px] border-[#e5e7eb]">
              {history.map((item, index) => {
                ...
                const visualStatus = getDiagnosticStatusVisualStatus(item.newStatus, approval.domainCode);
                ...
                return (
                  <li key={item.historyId} className={`ml-[20px] ${index !== history.length - 1 ? 'pb-[24px]' : ''}`}>
                    ...
                    <h3 className="flex items-center gap-[6px] mt-[6px] mb-[2px]">
                      <span className="font-title-small text-[var(--color-text-primary)]">{getDiagnosticStatusLabel(item.newStatus, approval.domainCode)}</span>
...
function SlotResultCard({ result, clarificationMessage }: { result: SlotResultDetail; clarificationMessage?: string }) {
  ...
  const hasReasons = result.reasons && result.reasons.length > 0;
  const [open, setOpen] = useState(false);
  const strippedMessage = humanizeSlotText(
    clarificationMessage?.replace(/^안녕하세요[^\n]*\n\n/, '').trim()
  );
