...
export default function DocumentReviewPage({ userRole }: DocumentReviewPageProps) {
  ...
  const analysisExtras = aiResult?.details?.extras;
  const finalRecognitionResult = (analysisExtras?.recognition_result as string | undefined) ?? aiResult?.whySummary;
  const finalFailureReasons = analysisExtras?.failure_reasons as string | undefined;
  const finalFailureExplanation = (
    (analysisExtras?.failure_explanation as string | undefined)
    ?? (analysisExtras?.service_why as string | undefined)
  );
          ...
          {aiResult && (
            <div className="bg-white rounded-[16px] border border-[#dee2e6] p-[24px] mb-[24px]">
              ...
              <div className="mb-[16px] p-[14px] rounded-[10px] border border-[#e9ecef] bg-[#f8f9fa]">
                ...
                {finalFailureReasons && (
                  <p className="font-body-small text-[#495057] mt-[8px] whitespace-pre-line">
                    실패 사유: {finalFailureReasons}
                  </p>
                )}
                {finalFailureExplanation && (
                  <p className="font-body-small text-[#495057] mt-[4px] whitespace-pre-line">
                    설명: {finalFailureExplanation}
                  </p>
                )}
              </div>
              ...
              <div className="mt-[16px] pt-[16px] border-t border-[#dee2e6]">
                <p className="font-body-small text-[#868e96]">
                  분석 일시: {formatKoreanDateTime(aiResult.analyzedAt)}
                </p>
              </div>
            </div>
          )}
            ...
            <div className="bg-white rounded-[16px] border border-[#dee2e6] p-[24px] mb-[24px]">
              ...
              <ol className="relative border-l-[2px] border-[#e5e7eb]">
                {history.map((item, index) => {
                  ...
                  const visualStatus = getDiagnosticStatusVisualStatus(item.newStatus, review.domainCode);
                  ...
                  return (
                    <li key={item.historyId} className={`ml-[20px] ${index !== history.length - 1 ? 'pb-[24px]' : ''}`}>
                      ...
                      <time className="inline-flex items-center px-[8px] py-[3px] mb-[6px] font-label-xsmall rounded-full"
                        ...
                        {formatKoreanDateTime(item.timestamp, { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </time>
                      <h3 className="flex items-center gap-[6px] mt-[6px] mb-[2px]">
                        <span className="font-title-small text-[#212529]">{getDiagnosticStatusLabel(item.newStatus, review.domainCode)}</span>
      ...
      {(() => {
        ...
        return readableSummary ? (
          <div className="px-[20px] pb-[16px]">
            <p className="font-body-small text-[#495057] leading-[1.6] whitespace-pre-line">{readableSummary}</p>
          </div>
        ) : null;
      })()}
    </div>
