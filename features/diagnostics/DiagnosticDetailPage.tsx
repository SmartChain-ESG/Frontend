...
import { getReasonLabel } from '../../src/constants/reasonLabels';
import { getSlotDisplayName, humanizeSlotText } from '../../src/constants/slotLabels';
import { formatKoreanDate, formatKoreanDateTime } from '../../src/utils/dateTime';
import { getDiagnosticStatusLabel, getDiagnosticStatusVisualStatus } from '../../src/utils/diagnosticStatus';
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-[24px] p-[24px] lg:p-[40px] max-w-[900px] mx-auto w-full">
        ...
        <div className="flex items-start justify-between gap-[16px]">
          ...
          <span className={`shrink-0 inline-block px-[12px] py-[6px] rounded-full font-title-xsmall border ${STATUS_STYLES[getDiagnosticStatusVisualStatus(diagnostic.status, diagnostic.domain?.code)] || 'bg-gray-50 text-gray-700 border-gray-200'}`}>
            {getDiagnosticStatusLabel(diagnostic.status, diagnostic.domain?.code)}
          </span>
        </div>
        ...
        <div className="bg-white rounded-[12px] border border-[var(--color-border-default)] p-[24px]">
          ...
          <div className="grid grid-cols-2 gap-[20px]">
            ...
            <InfoRow label="생성일" value={formatKoreanDate(diagnostic.createdAt)} />
            <InfoRow label="기안 시작일" value={formatKoreanDate(diagnostic.period?.startDate)} />
            <InfoRow label="기안 종료일" value={formatKoreanDate(diagnostic.period?.endDate)} />
            {diagnostic.submittedAt && (
              <InfoRow label="제출일" value={formatKoreanDate(diagnostic.submittedAt)} />
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-[12px] border border-[var(--color-border-default)] p-[24px]">
            ...
            <ol className="relative border-l-[2px] border-[#e5e7eb]">
              {history.map((item, index) => {
                ...
                const visualStatus = getDiagnosticStatusVisualStatus(item.newStatus, diagnostic.domain?.code);
                const statusConfig = TIMELINE_STATUS_CONFIG[visualStatus] || TIMELINE_STATUS_CONFIG.WRITING;
                ...
                return (
                  <li key={item.historyId} className={`ml-[24px] ${index !== history.length - 1 ? 'pb-[32px]' : ''}`}>
                    ...
                    <time className="inline-flex items-center px-[10px] py-[4px] mb-[8px] font-detail-small rounded-full"
                      ...
                      {formatKoreanDateTime(item.timestamp, {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        hour: '2-digit',
                    ...
                    <h3 className="flex items-center gap-[8px] mt-[8px] mb-[4px]">
                      <span className="font-title-small text-[var(--color-text-primary)]">
                        {getDiagnosticStatusLabel(item.newStatus, diagnostic.domain?.code)}
                      </span>
                      {isLatest && (
  ...
  return (
    <div className="rounded-[12px] border overflow-hidden" style={VERDICT_CARD_BG[verdict]}>
      ...
      {(() => {
        ...
        const readableSummary = humanizeSlotText(summaryText);
        return readableSummary ? (
          <div className="px-[20px] pb-[16px]">
            <p className="font-body-small text-[var(--color-text-secondary)] leading-[1.6] whitespace-pre-line">{readableSummary}</p>
          </div>
        ) : null;
      })()}
    </div>
...
function SlotResultCard({ result, clarificationMessage }: { result: SlotResultDetail; clarificationMessage?: string }) {
  ...
  const strippedMessage = clarificationMessage
    ?.replace(/^안녕하세요[^\n]*\n\n/, '')
    .replace(/[a-z]+(?:\.[a-z_]+)+/gi, displayName)
    .replace(/\(([A-Z0-9_]+)\)/g, (_: string, code: string) => `(${getReasonLabel(code)})`)
    .trim();
