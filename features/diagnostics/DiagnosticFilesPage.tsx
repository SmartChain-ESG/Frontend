...
import { previewAiRun, getAiHistory } from '../../src/api/aiRun';
import type { SlotStatus, SlotHint, RunPreviewResponse } from '../../src/api/aiRun';
import type { ErrorResponse } from '../../src/types/api.types';
import { getSlotDisplayName } from '../../src/constants/slotLabels';
import { formatKoreanDateTime, parseServerDate } from '../../src/utils/dateTime';
...
const AI_ANALYSIS_TIMEOUT_MS = 600000;
  ...
  return (
    <div className="space-y-[8px]">
      {slots.map((slot, index) => (
        <SlotCheckItem
          key={index}
          slotName={getSlotDisplayName(slot.slot_name, slot.display_name)}
      ...
      <div className="flex items-center gap-[12px]">
        ...
        <div>
          ...
          <p className="font-body-medium text-[var(--color-text-primary)]">
            {formatKoreanDateTime(parsingResult.completedAt, {
              year: 'numeric',
              month: '2-digit',
              day: '2-digit',
              hour: '2-digit',
...
export default function DiagnosticFilesPage() {
  ...
  const expectedPackageIdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (isAnalyzing && aiResult) {
      ...
      const analyzedAtMs = aiResult.analyzedAt ? parseServerDate(aiResult.analyzedAt).getTime() : NaN;
      const prevId = prevResultIdRef.current;
      const expectedPackageId = expectedPackageIdRef.current;

      if (expectedPackageId && aiResult.packageId === expectedPackageId) {
        setIsAnalyzing(false);
        prevResultIdRef.current = null;
        submitStartedAtRef.current = null;
        expectedPackageIdRef.current = null;
        navigate(`/diagnostics/${diagnosticId}`);
        return;
      }

      // Submit 이후 시각의 결과가 아니면 아직 새 결과가 아님.
      if (startedAt && Number.isFinite(analyzedAtMs) && analyzedAtMs < startedAt - 2000) {
        return;
      }

      // 이전 결과와 같은 ID면 skip (아직 새 결과 아님)
      if (!startedAt && prevId !== null && aiResult.id === prevId) {
        return;
      }

      // analyzedAt을 신뢰할 수 없는 경우에는 기존 ID 기준을 fallback으로 사용
      if (startedAt && !Number.isFinite(analyzedAtMs) && prevId !== null && aiResult.id === prevId) {
        return;
      }

      // baseline ID를 못 잡은 경우에도 submit 이전 결과면 skip
      setIsAnalyzing(false);
      prevResultIdRef.current = null;
      submitStartedAtRef.current = null;
      expectedPackageIdRef.current = null;
  ...
  // 결과가 오래 오지 않으면 polling을 중단해 무한 반복을 방지한다.
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      submitStartedAtRef.current = null;
      expectedPackageIdRef.current = null;
      toast.error('AI 분석이 지연되고 있습니다. 잠시 후 다시 시도해 주세요.');
  ...
  const getAutoTagForFile = useCallback((fileId: number): string | undefined => {
    ...
    return hint ? getSlotDisplayName(hint.slot_name, hint.display_name) : undefined;
  ...
  const handleSubmitAiRun = async () => {
    setShowSubmitModal(false);
    submitStartedAtRef.current = Date.now();
    expectedPackageIdRef.current = packageIdRef.current;

    // submit 직전 서버 기준 최신 결과 ID를 baseline으로 고정
    try {
      const history = await getAiHistory(diagnosticId);
      prevResultIdRef.current = history.length > 0
        ? history.reduce((maxId, item) => (item.id > maxId ? item.id : maxId), history[0].id)
        : null;
    } catch {
      const cached = queryClient.getQueryData(QUERY_KEYS.AI_RUN.RESULT(diagnosticId)) as { id?: number } | null;
      prevResultIdRef.current = aiResult?.id ?? cached?.id ?? null;
    }
