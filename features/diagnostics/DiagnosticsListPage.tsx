...
function formatDate(dateString?: string): string {
  ...
  return formatKoreanDate(dateString, { year: 'numeric', month: '2-digit', day: '2-digit' });
}
...
export default function DiagnosticsListPage() {
  ...
  const validDrafterStatuses = domainCode === 'ESG'
    ...
    : ['WRITING', 'APPROVED', 'COMPLETED', 'RETURNED'];

  const [statusFilter, setStatusFilter] = useState<StatusFilter>(() => {
    if (urlStatus === 'REVIEWING' && isSafetyOrComplianceDomain(domainCode)) {
      return 'APPROVED';
    }
  ...
  const statusTabs: { key: StatusFilter; label: string }[] = useMemo(() => {
    if (domainCode === 'ESG') {
      return [
        ...
        { key: 'SUBMITTED', label: '심사중' },
        { key: 'REVIEWING', label: '원청 심사중' },
    ...
    return [
      { key: 'ALL', label: '전체' },
      { key: 'WRITING', label: '작성중' },
      { key: 'APPROVED', label: '심사중' },
      { key: 'COMPLETED', label: '승인됨' },
      { key: 'RETURNED', label: '반려됨' },
    ];
  }, [domainCode]);
  ...
  const renderTableBody = () => {
    ...
    return items.map((item) => (
      ...
      >
        ...
        <td className="px-[16px] py-[14px] text-center">
          <span className={`inline-block px-[10px] py-[4px] rounded-full font-title-xsmall border ${STATUS_STYLES[getDiagnosticStatusVisualStatus(item.status, domainCode)] || 'bg-gray-50 text-gray-700 border-gray-200'}`}>
            {getDiagnosticStatusLabel(item.status, domainCode)}
          </span>
        </td>
      </tr>
    ));
